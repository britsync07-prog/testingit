  ...
  async function performSearch(headless: boolean): Promise<SearchResponse> {
    ...
    try {
      ...
      while (currentPage < maxPages) {
        ...
        const pageResults = await page.evaluate((maxResults: number): SearchResult[] => {
          const pageRes: { title: string; link: string; snippet: string }[] = [];
          ...
          for (const selectors of selectorSets) {
            if (pageRes.length >= maxResults) break;
            const containers = document.querySelectorAll(selectors.container);
            for (const container of containers) {
              if (pageRes.length >= maxResults) break;
              const titleElement = container.querySelector(selectors.title);
              ...
              let link = '';
              const linkInTitle = titleElement.querySelector('a');
              if (linkInTitle) { link = linkInTitle.href; }
              else {
                let cur: Element | null = titleElement;
                while (cur && cur.tagName !== 'A') cur = cur.parentElement;
                if (cur instanceof HTMLAnchorElement) link = cur.href;
                else { const cl = container.querySelector('a'); if (cl) link = (cl as HTMLAnchorElement).href; }
              }
              if (!link || !link.startsWith('http') || seenUrls.has(link)) continue;
              let snippet = '';
              const se = container.querySelector(selectors.snippet);
              if (se) snippet = (se.textContent || "").trim();
              if (title && link) { pageRes.push({ title, link, snippet }); seenUrls.add(link); }
            }
          }
