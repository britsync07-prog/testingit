...
function getHostMachineConfig(userLocale) {
    ...
    // 其他设置使用合理的默认值
    const reducedMotion = "no-preference"; // 大多数用户不会启用减少动画
    const forcedColors = "none"; // 大多数用户不会启用强制颜色
    // 选择一个合适的设备名称
    // 根据操作系统选择合适的浏览器
...
export async function googleSearch(query, options = {}, existingBrowser) {
    // 设置默认选项
    const { limit = 10, timeout = 60000, stateFile = "./browser-state.json", noSaveState = false, locale = "zh-CN", // 默认使用中文
     } = options;
    // 提取headless参数 (注意在commander中--no-headless会转换成false)
    ...
    if (fs.existsSync(stateFile)) {
        logger.info({ stateFile }, "发现浏览器状态文件，将使用保存的浏览器状态以避免反机器人检测");
    ...
    async function performSearch(headless) {
        ...
        try {
            ...
            if (isBlockedPage) {
                throw new Error("Captcha detected. Google search blocked.");
            }
            ...
            if (!resultsFound) {
                ...
                if (isBlockedDuringResults) {
                    throw new Error("Captcha detected. Google search blocked.");
                    if (headless) {
                        logger.warn("等待搜索结果时检测到人机验证页面，将以有头模式重新启动浏览器...");
                        // 关闭当前页面和上下文
                        await page.close();
                        await context.close();
                        // 如果是外部提供的浏览器，不关闭它，而是创建一个新的浏览器实例
                        ...
                        logger.warn("等待搜索结果时检测到人机验证页面，请在浏览器中完成验证...");
                        // 等待用户完成验证并重定向回搜索页面
                        await page.waitForNavigation({
                            timeout: timeout * 2,
                            url: (url) => {
            ...
            let results = []; // 在 evaluate 调用之前声明 results
            ...
            logger.info({ count: results.length }, "成功获取到第1页搜索结果");
            // --- PAGINATION: click Next to collect more pages ---
            const maxPages = options.pages || 1;
            let currentPage = 1;
            while (currentPage < maxPages) {
                // Check for captcha before paginating
                const pageUrlNow = page.url();
                if (sorryPatterns.some((p) => pageUrlNow.includes(p))) {
                    logger.warn("Captcha detected during pagination.");
                    throw new Error("Captcha detected. Google search blocked.");
                }
                ...
                logger.info({ page: currentPage + 1 }, "Clicking Next page...");
                await nextBtn.click();
                await page.waitForLoadState("networkidle", { timeout });
                // Check for captcha after navigation
                const nextPageUrl = page.url();
                if (sorryPatterns.some((p) => nextPageUrl.includes(p))) {
                    throw new Error("Captcha detected. Google search blocked.");
                }
                // Small human-like pause
                await page.waitForTimeout(getRandomDelay(500, 1200));
                // Extract results from this page (reuse same evaluation logic)
                const pageResults = await page.evaluate((maxResults) => {
                    const pageRes = [];
                    ...
                    for (const selectors of selectorSets) {
                        ...
                        for (const container of containers) {
                            ...
                            if (linkInTitle) {
                                ...
                                while (cur && cur.tagName !== 'A')
                                    cur = cur.parentElement;
                                if (cur instanceof HTMLAnchorElement)
                                    link = cur.href;
                                else {
                                    const cl = container.querySelector('a');
                                    if (cl)
                                        link = cl.href;
                                }
                            }
                            if (!link || !link.startsWith('http') || seenUrls.has(link))
                                continue;
                            let snippet = '';
                            const se = container.querySelector(selectors.snippet);
                            if (se)
                                snippet = (se.textContent || "").trim();
                            if (title && link) {
                                pageRes.push({ title, link, snippet });
                                seenUrls.add(link);
                            }
                        }
                    }
                    return pageRes.slice(0, maxResults);
                }, limit);
                results.push(...pageResults);
                logger.info({ page: currentPage + 1, count: pageResults.length }, "Got results from next page");
            ...
            // --- END PAGINATION ---
            logger.info({ total: results.length }, "成功获取到搜索结果");
            try {
                // 保存浏览器状态（除非用户指定了不保存）
                if (!noSaveState) {
...
export async function getGoogleSearchPageHtml(query, options = {}, saveToFile = false, outputPath) {
    ...
    const { timeout = 60000, stateFile = "./browser-state.json", noSaveState = false, locale = "zh-CN", // 默认使用中文
     } = options;
