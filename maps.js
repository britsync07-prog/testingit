...
class BusinessScraper {
  ...
  async scrapeGoogleMaps(searchQuery, maxResults = 30) {
    ...
    try {
      // ?hl=en forces English so the DOM doesn't change based on your location
      ...
      const businesses = await page.evaluate(() => {
        ...
        for (let i = 0; i < businessCards.length; i++) {
          ...
          infoContainers.forEach(container => {
            ...
            // Updated Regex: Handles 11+ digit continuous strings and 5-6 digit formatting blocks
            const phoneMatch = text.match(/(\+\d{1,4}[\s.-]?)?(\(?\d{2,6}\)?[\s.-]?)?(\d{2,6}[\s.-]?){1,4}\d{2,6}/);
            
            if (phoneMatch && text.includes(phoneMatch[0])) {
              // Ensure we aren't accidentally grabbing a zip code
              if (phoneMatch[0].replace(/\D/g, '').length >= 7) {
                phone = phoneMatch[0];
              }
            }
            
            // Extract address (usually separated by a dot and doesn't contain parenthesis like review counts)
      ...
      console.log(`Successfully extracted ${businesses.length} raw businesses from Maps.`);
  ...
  // Universal phone cleaner (keeps only digits and the + sign)
  ...
  async findEmails(websiteUrl) {
    ...
    let page;
    try {
      console.log(`  -> Searching for emails on: ${websiteUrl}`);
      ...
      console.log(`  -> Failed to extract from ${websiteUrl}: ${error.message}`);
      return [];
    } finally {
      if (page) await page.close();
    }
  ...
  async processResults(targetCount = 20) {
    console.log("\nProcessing results and finding emails... (This might take a moment)");
